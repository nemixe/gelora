  ...
  <div>
    <div
      ...
      <div ref="slider" class="absolute images z-0 h-full w-full">
        <img class="h-full w-full object-cover inline-block" :class="{ 'active': index === 0 }" src="https://gedubar.com/wp-content/uploads/2017/08/basket-ball-1.jpg" />
        <img class="h-full w-full object-cover inline-block" :class="{ 'active': index === 1 }" src="https://www.gannett-cdn.com/presto/2019/12/16/USAT/bda112e2-efd0-4464-8b24-7d6b3f0224a7-USATSI_13793959.jpg" />
        <img class="h-full w-full object-cover inline-block" :class="{ 'active': index === 2 }" src="https://static01.nyt.com/images/2019/09/11/sports/11basketball/11basketball-articleLarge.jpg?quality=75&auto=webp&disable=upscale" />
      </div>
      <div class="absolute z-10 bg-banner w-full h-full"></div>
      ...
      <div class="relative select-none z-10 h-56 w-full">
        <div class="flex justify-between items-center w-full h-full">
          <div @click="moveSlide(index-1)" class="cursor-pointer text-gray-700 text-center p-4" :class="{ 'invisible': index === 0 }">
            <img class="w-4" src="arrow-left.svg" alt="" srcset="">
          </div>
          <div @click="moveSlide(index+1)" class="cursor-pointer text-gray-700 text-center p-4" :class="{ 'invisible': index === 2 }">
      ...
      <div class="absolute z-20 bottom-0 px-5 pb-8 w-full">
        <div class="w-full flex justify-center py-2">
          <div class="h-1 w-1 ml-1 bg-white rounded-lg inline-block" :class="{ 'w-3': index === 0 }"></div>
          <div class="h-1 w-1 ml-1 bg-white rounded-lg inline-block" :class="{ 'w-3': index === 1 }"></div>
          <div class="h-1 w-1 ml-1 bg-white rounded-lg inline-block" :class="{ 'w-3': index === 2 }"></div>
        </div>
        <div class="tags mb-1 font-size-1">
          <div class="bg-black text-white tag px-2 inline-block">Basket</div>
          <div class="bg-black text-white tag px-2 inline-block">Fun</div>
        </div>
...
export default {
  ...
  methods: {
    ...
    onTouchEnd: function(e) {
      ...
      this.index = Math.round(scrollLeft / this.width)

      if(diff > 50 && (this.walk > 60 || this.walk < -60)) {
        this.index = this.walk > 60 ? this.index - 1 : this.index + 1
        this.moveSlide(this.index)
      }
      else this.moveSlide(this.index)
    },
    moveSlide: function(index) {
      if(index <= 0) this.index = 0
      else if(index >= 2) this.index = 2
      else this.index = Math.round(index)
